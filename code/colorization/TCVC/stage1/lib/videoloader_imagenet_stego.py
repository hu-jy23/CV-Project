...
class VideosDataset_ImageNet_10k(data.Dataset):
    def __init__(
        ...
        #----gpt----
        for video_name, frame_list, loadmode in curr_video_pairs:
            for frame_path in frame_list:
                image_pairs.append((video_name, frame_path, loadmode))  # frame_path 是 str 而不是 list

        if not image_pairs:
            raise RuntimeError("Found 0 image pairs in all the data_roots")
    ...
    def __getitem__(self, index):
        try:
            ...
            video_name, img_path, loadmode = self.image_pairs[index]

            I1,flag = pil_loader_new(img_path)
            if flag == 0:
                raise ValueError("Gray image, skipping")        
