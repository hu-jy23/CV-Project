...
def interpolate_all_pairs(model, input_folder, output_folder, device='cuda'):
    ...
    for i in tqdm(range(len(images) - 1), desc="Interpolating"):
        img0, _ = read_image(images[i], device=device)
        img1, _ = read_image(images[i + 1], device=device)
        dt = torch.tensor([[0.5]], dtype=torch.float32, device=device)
        print(f"img0 device: {img0.device}, img1 device: {img1.device}, dt device: {dt.device}")
