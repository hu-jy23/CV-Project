...
def get_args_parser():
    ...
    parser.add_argument('--test_path', type=str,default='./dataset/temp/test_input/test_input_new')  #/dataset/videvo/test/imgs /dataset/DAVIS/Test/1/imgs

    parser.add_argument('--test_output_path', type=str,default='./stage2_test_results_xll_new') 
    parser.add_argument('--device', default='cuda',
                        help='device to use for training / testing')
    
    parser.add_argument("--img_size", type=int, default=[904, 1618] )  
    ...
    parser.add_argument('--ref_path', default='./stage1_test_results_xll',)  #./stage1_test_results
...
def main_testonly_parallel(args):
    ...
    key=lambda x: int(x.split('_')[1]))  # 提取 "set_1" -> "1" 并转为 int
    # print 什么情况
    ...
    if args.ref_path:
        reference_imgs = glob.glob(os.path.join(args.ref_path,"*.JPEG")) + glob.glob(os.path.join(args.ref_path,"*.png"))
        # 按文件名中的数字部分排序
        reference_imgs.sort(key=lambda f: int("".join(filter(str.isdigit, os.path.basename(f)))))
        print("reference_imgs (sorted by number):", reference_imgs)
        print("len(reference_imgs):", len(reference_imgs))
        print("args.ref_path:", args.ref_path)

    if args.ref_path and len(subdirs) != len(reference_imgs):
        raise ValueError(f"错误：参考帧数量({len(reference_imgs)})与测试目录数量({len(subdirs)})不匹配")
