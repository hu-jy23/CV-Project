...
parser.add_argument("--workers", type=int, default=1)    #win=1, linux=4
parser.add_argument("--batch_size", type=int, default=1)   #不分布，batch小
parser.add_argument("--image_size", type=int, default=[432, 768])
...
parser.add_argument("--test_video_size", type=int, default=[904,1618]) 
parser.add_argument("--ic", type=int, default=4)
parser.add_argument("--epoch", type=int, default=40)
parser.add_argument("--color_prior", type=int, default=[0.9621, 0.2731, 0.7017, 0.1786])  #[0.8, 0.6, 0.4, 0.3]  [0.5, 0.5, 0.5, 0.5]
                                                                                #[0.9621, 0.2731, 0.7017, 0.1786] [0.731, 0.3866, 0.6009, 0.3393]
parser.add_argument("--resume_iter", type=int, default=342000)
...
parser.add_argument("--test_video_output_path", type=str, default="./stage1_test_results_xll")

parser.add_argument('--local_rank', default=-1, type=int,help='node rank for distributed training')

def gpu_setup_DDP():
    cudnn.benchmark = True
    #device = torch.device("cuda",opt.local_rank)    #不要分布式
...
def resume_model():									   #继续学习 ，加载参数
    ...
    if opt.resume_iter:
        ...
        colornet.load_state_dict(checkpoint, strict=opt.strict_load)
    return
...
def test_video():
    ...
    for iter,data in enumerate(tqdm(val_dataset)):
        ...
        for i in range(len(I_current_l)):
            ...
            save_frames(I_current_rgb, video_output_path, image_name = f"{subdir}_{iter:03d}.png")
